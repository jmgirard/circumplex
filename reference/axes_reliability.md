...
noise-dominance reading is calibrated for `n` up to about `5e5`):
...
for a typical design at `n` up to about `5e5`. Above that the guarantee
is the fixed target alone, not noise dominance. The derivation and the
