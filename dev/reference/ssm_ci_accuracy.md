...
to the pooled within-group scale correlations; each of `reps` datasets
is then simulated from that plug-in population at the object's exact
group sizes and the object's own interval procedure (same engine, same
