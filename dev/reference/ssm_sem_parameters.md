...
take an already fitted lavaan model of a fixed-angle circumplex
measurement structure (as generated by
...
applies. Where
[`ssm_sem()`](http://circumplex.jmgirard.com/dev/reference/ssm_sem.md)
fits a configural-metric-scalar ladder and refuses a latent group
contrast when the required rung is rejected, `ssm_sem_parameters()`
computes the contrast from whatever multi-group fit you supply without
testing invariance at all. You own the comparability claim: the groups
are compared on this instrument's latent metric only to the extent the
model you fitted makes them comparable.
...
[`ssm_sem()`](http://circumplex.jmgirard.com/dev/reference/ssm_sem.md)
for reproducible confidence intervals.
