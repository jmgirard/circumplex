...
wrap correctly rather than spanning “the long way around.” For plotting
or reading the trend, unwrap the estimated trajectory onto a continuous
branch with
[`angle_unwrap()`](http://circumplex.jmgirard.com/dev/reference/angle_unwrap.md):

``` r

d_branch <- angle_unwrap(trajectory$d_est)
# Each credible bound is reported independently wrapped into [0, 360), so an
# interval straddling the 0/360 seam has d_lci > d_uci. Move each bound onto
# the estimate's unwrapped branch by its *signed* distance from the estimate
# (not by the estimate's branch offset, which would throw a straddling bound
# a full turn off).
rel <- function(a) ((a - trajectory$d_est + 180) %% 360) - 180
plot_dat <- data.frame(
  wave = waves,
  d = d_branch,
  lci = d_branch + rel(trajectory$d_lci),
  uci = d_branch + rel(trajectory$d_uci)
)
ggplot(plot_dat, aes(x = wave, y = d)) +
  geom_ribbon(aes(ymin = lci, ymax = uci), alpha = 0.25) +
  geom_line() +
  geom_point() +
  labs(x = "Wave", y = "Displacement (degrees, unwrapped)") +
  theme_minimal()
```

![](growth-ssm-analysis_files/figure-html/plot-1.png)

(Each interval endpoint is placed on the same branch as its estimate by
its signed circular distance from it; near the boundary the endpoints
may legitimately fall outside $`[0°, 360°)`$.)
