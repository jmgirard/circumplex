...
and its helpers expose it. The vignette teaches two products: the latent
...
- The angles are **theoretical claims, not estimates**.
  ...
  It fits Browne’s (1992) circumplex model. Examining geometry is a
...
directly, because
[`ssm_sem()`](http://circumplex.jmgirard.com/dev/reference/ssm_sem.md)
does. But it is the escape hatch for respecifications that you then fit
...
By default
[`ssm_sem()`](http://circumplex.jmgirard.com/dev/reference/ssm_sem.md)
fits with a robust estimator (`estimator = "MLR"`, maximum likelihood
with robust corrections) and reports robust global fit indices. This is
