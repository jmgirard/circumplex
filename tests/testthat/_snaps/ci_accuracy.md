      ...
      SSM CI accuracy, simulated at your n and settings (30 replications per condition; bootstrap intervals with 60 replicates at level 0.95)
        ...
        # Profile [All] (n = 120; 95% bootstrap CIs, 60 replicates):
          ...
          Guardrail      under a truly zero amplitude, displacement would be certified
                         0.0% of the time (user-expectation benchmark 2.5%)
    ...
    Output
      ...
      SSM CI accuracy, simulated at your n and settings (12 replications per condition; bootstrap intervals with 60 replicates at level 0.95)
        ...
        # Profile [Female] (n = 118; 95% bootstrap CIs, 60 replicates):
          ...
          Guardrail      if the true amplitude were zero, displacement would still be
                         certified 16.7% of the time, far more often than the 2.5%
                         error rate the guardrail's wording suggests
        Verdict: CAUTION. Amplitude CIs are less reliable than nominal at this sample
        size and the interpretability guardrail certifies a truly zero amplitude more
        often than its wording suggests. Elevation and certified displacement coverage
        rates are borderline at this number of replications; a larger `reps` would
        sharpen the verdict. Consider a larger sample or treat near-zero amplitudes as
        inconclusive rather than absent.
      
        # Profile [Male] (n = 122; 95% bootstrap CIs, 60 replicates):
          Elevation      coverage 91.7%: borderline
          Amplitude      coverage 75.0%: INADEQUATE (under-coverage; misses are almost
          ...
          Guardrail      if the true amplitude were zero, displacement would still be
                         ...
                         error rate the guardrail's wording suggests
        Verdict: CAUTION. Amplitude CIs are less reliable than nominal at this sample
        size, displacement CIs mis-cover even when certified, and the interpretability
        guardrail certifies a truly zero amplitude more often than its wording
        suggests. Elevation coverage is borderline at this number of replications; a
        larger `reps` would sharpen the verdict. Consider a larger sample or treat
        ...
        Verdict: CAUTION. Amplitude CIs are less reliable than nominal at this sample
        size. Elevation and displacement coverage rates are borderline at this number
        of replications; a larger `reps` would sharpen the verdict. Consider a larger
