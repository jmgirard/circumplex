...
- arm64 CRAN flavor: `tools/arm64/check.sh <tarball>` — reproduces CRAN's
  linux-arm64 additional check locally (Docker, native on Apple Silicon). Green
  is required before any CRAN submission; `tools/arm64/README.md` says what the
...
- Displacement CIs use circular quantiles (center on circular mean, unwrap,
  ...
  near the boundary; see cairn/DESIGN.md).
...
- Closed-form SSM estimator equals OLS **for equally spaced angles** (exact
  ...
  necessary; cairn/DESIGN.md). The SEM layer (`ssm_sem*`) always uses the OLS
  projection instead.
...
2. **Implement test-first** (testthat; a regression test reproducing the bug
   before the fix).
3. **Validate estimation-code changes** (`ssm_*` statistics, `src/`) against an
   independent oracle. The repo-local `/statistical-validation` skill was
   entombed by the cairn migration to `cairn/legacy/statistical-validation/`;
   its validation battery still applies, and cairn's oracle doctrine
   (`tracking-rules.md` "Validation doctrine") reinforces it.
4. **Review** with `/code-review` before committing; `/code-review max` for a
   statistically risky release. The billed cloud `/code-review ultra` is
   reserved for a flagship release and only when the user asks.
...
Opus for general implementation; Sonnet for mechanical edits, doc updates, and
running checks. **Always recommend a tier when proposing or handing off a
task**, with a one-line why mapped to the task's risk.

The pre-cairn skills `/next-task` and `/release-checklist` were entombed to
`cairn/legacy/`; their cairn-era replacements are `/milestone` (status/what's
next) and `/cairn-release` (CRAN release walk). Project status now lives in
`cairn/ROADMAP.md`, not this file — see the Project tracking section below.
