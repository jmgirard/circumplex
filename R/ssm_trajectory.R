...
#' \[0, 360). The lower confidence bound is placed below the estimate by the
#' counterclockwise angle from the lower bound to the estimate, and the upper
#' bound is placed above the lower bound by the counterclockwise angle from the
#' lower bound to the upper bound. An interval is therefore always drawn as the
#' counterclockwise arc from its lower to its upper bound, even when that arc
#' is wider than a half-turn. Unwrapping assumes the profile rotates less than a
