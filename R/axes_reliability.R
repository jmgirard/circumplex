...
#' is refused by both surfaces. Which degeneracies those are is settled in two
#' steps. A matrix whose smallest eigenvalue, relative to its largest, falls
...
#' there. That committed counterexample is the three-variable matrix saved in
#' the package sources as `tests/testthat/fixtures/rb18-counterexample-b.rds`.
#' On it, the check's worst estimate differed between macOS (arm64, R's
#' reference BLAS and LAPACK) and Linux (arm64, OpenBLAS). That estimate is the
#' figure the warning prints, and both machines refused the fit as
#' `"uncertified"`. The three estimates are read as one because both
...
#' target is set from two channels that
...
#' uncorrected normal-theory pricing, kept only as a diagnostic tie to
#' lavaan's own standard errors -- inverts. A matrix degenerate only in the
#' raw metric (wildly unequal fitted variances over a well-conditioned
#' correlation structure) refuses that internal arm alone: the reported
#' standard errors and scaled fit statistics all compute --
#' `details$se_correction_failed` and `details$fit_scaling_failed` are both
...
#'   statement about the model, and `"uncertified"` where the check could not
#'   place this fit's numbers inside the accuracy target, a numerical caution
#'   whose warning names the estimated relative error --
