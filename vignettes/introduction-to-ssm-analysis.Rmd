...
```{r model, echo = FALSE, fig.width = 7.2, fig.height = 4}
...
```{r column, echo = FALSE, fig.width = 7.2, fig.height = 4}
...
```{r path, echo = FALSE, fig.width = 7.2, fig.height = 4}
dat_r <- tibble(
...
```{r curve, echo = FALSE, fig.width = 7.2, fig.height = 4}
...
```{r residuals, echo = FALSE, fig.width = 7.2, fig.height = 4}
...
```{r iipsc, echo = FALSE, fig.width = 7.2, fig.height = 4}
angles <- c(90, 135, 180, 225, 270, 315, 360, 45)
flabel <- c(
  "Domineering",
  "Vindictive",
...
need to be combined into a vector using the `c()` function. The order
of the `scales` and `angles` vectors needs to be the same, and so does the length.
...
the table, simply pass the `results` (or `results2`) object to the `ssm_table()` function.
...
```{r plot1, fig.width = 7.2, fig.height = 4}
ssm_plot(results2)
```
...
analysis. All SSM analyses use the `ssm_analyze()` function and the data, scales, and
...
```{r plot2, fig.width = 7.2, fig.height = 4}
ssm_plot(results3)
```
