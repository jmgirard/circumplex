...
Occasions are connected in the order the rows appear in the data. When you
assemble a data frame by hand, the optional `order` aesthetic sorts within each
series, which is the safer choice for the reason noted above: sorted as text,
`T10` lands before `T2` and silently reverses time. Mapping `group` draws one
path per series.
