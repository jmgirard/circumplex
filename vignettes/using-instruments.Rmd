...
To reduce loading time and memory usage, instrument information is not loaded into R's memory when the circumplex package is loaded. Instead, the `instrument()` function can be used to load specific instruments into memory. The instrument to be loaded is determined by the code argument (as below), which corresponds to the codes displayed by the `instruments()` function (as above). To save time, the `code =` and `<-` parts can be omitted and the instrument data will be loaded into an object with the same name as the instrument's code.

```{r}
# Load data into object called my_instrument
my_instrument <- instrument(code = "csip")
```

```{r}
# Shortcut to load data into object called csip
instrument("csip")
```
...
Special thanks to the authors and publishers who granted permission to include information about their instruments in this package: Michael Boudreaux, Christopher Hopwood, Kenneth Locke, Patrick Markey, Aaron Pincus, and MindGarden Inc.
