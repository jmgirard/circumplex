...
## 1. Overview
This vignette extends the basic SSM analyses to more than one group or measure. Section 2, "Generalizing the basic analyses", adds the `grouping` and `measures` arguments of `ssm_analyze()`. Section 3, "Contrast analyses", adds the `contrast` argument and `ssm_plot_contrast()`. Section 4, "Taxonomy of analysis types", lists every analysis type and the arguments that select it. Section 5, "Working with SSM tables", customizes the output of `ssm_table()`. Section 6, "Working with SSM figures", exports figures with `ggsave()`. The Wrap-up lists what the page covered and names the next pages, and the References list the sources cited.
...
## Wrap-up
In this vignette, we learned how to generalize the SSM analyses to multiple groups and measures, how to conduct contrast analyses, how to make basic customizations to tables and figures, and how to export tables and figures to external files. Two pages follow this one. "Evaluating Circumplex Structure" asks whether an instrument fits a circumplex, and the pages after it ask whether its confidence intervals can be trusted. "Bayesian SSM Analysis" estimates the same parameters with a Bayesian model.
