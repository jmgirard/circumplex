...
What it costs to *ignore* real blocks depends on their geometry, and the answer
is less alarming than it sounds. The general factor soaks up block variance
whichever way the blocks fall, so `xi2` is inflated whenever blocks exist and go
undeclared. The axes variance — the quantity reliability is actually read from —
only moves when blocks are *angularly clustered*, that is, when the items
sharing a block sit near one another on the circle. A block that draws about
evenly from around the circle is orthogonal to the axes' cosine term, and leaves
`xi1`, the reliability, and the SEm untouched. So a blockwise instrument whose
blocks sample the circle evenly gives a trustworthy reliability even without
`blocks`; one whose blocks cover contiguous arcs does not, and there the bias
