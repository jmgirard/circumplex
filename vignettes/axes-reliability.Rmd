...
the *uncorrected* values under the bare names `chisq`, `rmsea` and `cfi`. So
both obvious cross-checks — against lavaan's bare `cfi` and against its
...
is the N the fit was priced at — the count of rows the estimator was actually
handed, which is smaller than `details$n_total` wherever cases were dropped.
Divide the first by the second and read across.
