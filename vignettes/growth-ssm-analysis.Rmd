...
pointwise $d(t)$ coverage from the nominal 95% to roughly 84%, while the
...
shift <- d_branch - trajectory$d_est
plot_dat <- data.frame(
  wave = waves,
  d = d_branch,
  lci = trajectory$d_lci + shift,
  uci = trajectory$d_uci + shift
)
...
(The interval endpoints are shifted onto the same branch as their
estimate; near the boundary they may legitimately fall outside
$[0°, 360°)$.)
...
degrees-of-freedom-adjusted inference — the Kenward–Roger adjustment
(available for `lme4` fits via **pbkrtest**, or in `nlme`) or
$t$-quantile-based intervals — or a parametric bootstrap of the fixed
effects, over raw normal-approximation draws.
