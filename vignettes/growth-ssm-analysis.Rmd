...
that tells a joint fit apart from separate fits assembled afterward.
...
and ask whether every entry differs from zero.


``` r
all(V[c("dvx", "dvx:wave"), c("dvy", "dvy:wave")] != 0)
#> [1] TRUE
```

A joint fit estimates these entries, so they are nonzero. Separate fits
produce one covariance matrix per coordinate, and a full matrix assembled
from them block by block holds exact zeros in every cross block. The check
detects only that assembled structure. A nonzero block does not show that
the model is right: a misspecified joint model passes it too. Carry this
check to your own fits.
