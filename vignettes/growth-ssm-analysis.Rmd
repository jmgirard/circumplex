...
The person block of the variance components is where the fit is joint. Its
...
zero. Most of its entries here are tiny, and only the intercept covariance
is appreciable, which is why the check asks for any nonzero entry rather
than for every entry. Separate fits produce one covariance matrix per
coordinate, and a full matrix assembled from them block by block holds
exact zeros in every cross block. The check detects only that assembled
structure. A nonzero block does not show that the model is right: a
misspecified joint model passes it too. Carry this check to your own fits.
