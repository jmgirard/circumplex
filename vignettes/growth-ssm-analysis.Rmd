...
does not run the loop. The replicates it made ship as
`growth_bootstrap_draws.rds`, written by the loop above under that seed
and a rule that drops a refit whose fixed effects are not all finite. The
...
five waves is 0.001.
The bootstrap earns its cost at smaller samples than this one of 150
persons, where the REML intervals narrow the most.
