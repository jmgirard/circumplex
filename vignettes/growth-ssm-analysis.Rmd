...
model, with the person effects and the residuals both redrawn, and refits
the model to it. The replicate is that refit's fixed effects. The
replicates go into `ssm_trajectory()` as `draws =`, in place of `coef` and
`vcov`, and the intervals then come from the spread of the refits rather
than from the normal approximation. For glmmTMB the loop is
`simulate()` and `refit()`:
...
which runs that loop under that seed with a rule that drops and replaces a
refit whose fixed effects are not all finite (none were dropped). The
file has one row per replicate and one column per coefficient:
...
weight, which the Section 10 nlme call carries, so a user on nlme runs the
bootstrap through the glmmTMB call of Section 4. The two engines fit the
