...
model draws (random draws of the fitted model's estimates) on the way out
...
coverage drops from the nominal 95% to roughly 86%. The joint recipe stays at
nominal. Pointwise coverage is how often the interval at one wave contains
the true direction. Such a
design is realistic, because profile tilts are rarely aligned with an axis.
...
the boundary, which is stored with `d_lci > d_uci`, because each bound is
wrapped into $[0°, 360°)$ on its own.
...
model.
Degrees-of-freedom-adjusted inference widens intervals to account for a
small sample. It includes the Kenward–Roger adjustment for
