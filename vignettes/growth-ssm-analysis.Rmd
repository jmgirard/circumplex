...
group's interpersonal style drift toward warmth? Does its distinctiveness
...
This vignette presents the package's recommended recipe, which avoids the
boundary entirely. It models growth in the Cartesian coordinates $(x, y)$,
the same coordinates that the SSM estimator itself uses. At the end, it
converts fitted trajectories back to $(a(t), d(t))$ with circular-correct
summaries.
...
`nlme` (shipped with base R), using its `varIdent`/`corSymm` machinery.
...
We simulate five waves of octant scores for 150 persons. Their group-level
displacement drifts from 350° to 10°, deliberately crossing the 0°/360°
boundary. Their amplitude stays near 0.6 throughout.
...
outcome its own intercept and slope. Let the person-level random effects be
...
Separate fits have independent covariance matrices, which silently sets
$\mathrm{Cov}(\hat{x}(t), \hat{y}(t)) = 0$.

Our validation simulations used a design with strongly correlated $x$–$y$
person effects. This is a realistic situation, because profile tilts are
rarely axis-aligned. In that design, the shortcut's pointwise $d(t)$
coverage drops from the nominal 95% to roughly 86%. The joint recipe stays
at nominal. Do not fit the coordinates separately.

## 4. From fixed effects to $(a(t), d(t))$ with intervals

The fixed effects define the mean trajectory
$(\hat{e}(t), \hat{x}(t), \hat{y}(t))$. Three steps carry uncertainty
through the nonlinear map to $(a(t), d(t))$. First, draw coefficient vectors
from the multivariate normal implied by the fixed-effect covariance matrix.
This is the same asymptotic move that the package's Monte Carlo engine
makes. Second, evaluate each draw at each wave. Third, hand the per-wave
draws to `ssm_draws()`. It applies the package's circular statistics:
medians and circular means, and equal-tailed intervals with correct wrapping
at the boundary.
...
The displacement estimates hug the 0°/360° boundary by design. Values are
near 350° at early waves and near 10° at late waves. The intervals wrap
...
Values there may legitimately fall outside $[0°, 360°)$. Each interval is
placed on its estimate's branch and keeps the width it was reported with.
That includes an interval that straddles the seam, which is stored with
`d_lci > d_uci`.

Doing this by hand is easy to get subtly wrong. A natural recipe is to
"shift each bound by its signed distance from the estimate." That recipe
silently cannot represent an interval wider than a half-turn. Such an
interval is exactly the near-origin case that Section 5 is about. The
...
origin. As $a(t) \to 0$, the draws of $d(t)$ become diffuse or bimodal. A
quantile interval of them is then not a trustworthy statement about
direction.

At each wave, `ssm_draws()` applies the package's scale-free certification
rule to the amplitude interval. The rule requires the lower bound to be at
least 0.35 interval-widths above zero. `ssm_draws()` records the verdict in
...
In our validation simulations of this regime, the caution fires at the
degraded wave in essentially every replicate. Meanwhile, the origin-distal
...
estimated variance components. That is, it ignores their uncertainty. At
small sample sizes, this makes the resulting intervals anticonservative
(too narrow). This is a property of the mixed-model machinery, not of the
SSM transform, and the remedy is user-side.

With modest N, prefer one of two options over raw normal-approximation
draws. The first is degrees-of-freedom-adjusted inference. Examples are the
Kenward–Roger adjustment for `lme4` fits via **pbkrtest**, the approximate
denominator degrees of freedom that `nlme` supplies, or $t$-quantile-based
intervals. The second is a parametric bootstrap of the fixed effects.
...
shortest signed rotation between successive waves. By the package's
half-turn convention, an exact 180° step ascends. An `NA` makes every later
wave branch-ambiguous, so `NA` propagates onward. The unwrapped values live
on an ordinary line, so any univariate mixed model applies. This framing
models the *mean of the person-level directions*. That is a legitimate (and
different) estimand from the direction of the mean trajectory in Section 4.
...
* **Low amplitude.** Suppose a person's amplitude is near zero at some
  ...
  This is the same reason that the Section 5 certification exists.

The two recipes agree closely in the concentrated, common-branch regime. In
that regime, everyone is well away from the origin, trajectories move
slowly, and directions cluster. Our validation simulations there find mean
trajectory differences well under a degree. So the choice matters exactly
...
* **The derived $d(t)$ is the direction of the mean trajectory, not the
  ...
  questions. Section 7's unwrap recipe targets the latter.
...
large-sample approximation. It is defensible in the concentrated regime,
and the Section 5 certification guards it elsewhere. The fully model-based
upgrade is **projected-normal regression**, for example the **bpnreg**
package. It models circular outcomes directly with person-level structure
and gives exact posterior inference for $d(t)$. circumplex does not wrap
it. But `ssm_draws()` will happily summarize posterior draws produced by
any such model.
