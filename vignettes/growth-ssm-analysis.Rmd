...
names the next page, and the References list the sources cited.
...
wave and coordinate. `ssm_growth_data()` builds it. It scores each row of
the wide table as its own profile through `ssm_parameters_id()`, and it
...
`ssm_growth_data()`, and `"wave"` and `"person"` are their defaults. The
...
Before the first step, the call makes the check that Section 4 promised. It
...
so it is not zero. Separate fits produce one covariance matrix per
...
three calls are the same, and the fit reads the same two formulas:
...
of this near-origin case, the caution fires at the degraded wave in
...
it plots the same way, minus the hollow marking and its legend. The figure
then makes no claim about interpretability either way. That is the honest
default when the verdict was never computed.
...
person intercepts and the same residual variance per coordinate.

**nlme** ships with R. Its call sets `na.action = na.omit`, because
`nlme::lme()` otherwise stops on a row whose `value` is `NA`, while glmmTMB
drops such a row by default. Its fixed effects and their covariance go to
`ssm_trajectory()` exactly as glmmTMB's do. On the data of Section 3, its
fixed effects match glmmTMB's to within $10^{-13}$, and their covariance
matrix matches to within $10^{-8}$:
...
glmmTMB table's estimates. The printed caution is written for the `coef`
and `vcov` shape. A posterior interval from brms depends on its priors
