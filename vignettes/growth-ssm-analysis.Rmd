...
$d(t)$ intervals are not interpretable", and Section 7, "Small samples: the
REML caution and the bootstrap remedy", state the limits. Section 8, "The unwrap
...
## 7. Small samples: the REML caution and the bootstrap remedy
...
The caution under the Section 5 table states this for the `coef` and
`vcov` shape.

The remedy that enters the helper is a parametric bootstrap of the fixed
effects. Each replicate simulates a new response vector from the fitted
model, with the person effects and the residuals both redrawn. It then
refits the model to that vector. The replicate is that refit's fixed
effects. The replicates go into `ssm_trajectory()` as `draws =`, in place
of `coef` and `vcov`. The intervals then come from the spread of the
refits rather than from the normal approximation. For glmmTMB the loop is
`simulate()` and `refit()`:


``` r
ys <- simulate(fit, nsim = 1000, seed = 20260716)
boot <- t(sapply(ys, function(y) glmmTMB::fixef(glmmTMB::refit(fit, y))$cond))
```

A thousand refits of this model take about three minutes, so the page
does not run the loop. The replicates ship as
`growth_bootstrap_draws.rds`, written by `data-raw/growth-bootstrap-draws.R`.
That script runs the loop under that seed. It drops and replaces a refit
whose fixed effects are not all finite, and none were dropped. The file
has one row per replicate and one column per coefficient:


``` r
boot <- readRDS("growth_bootstrap_draws.rds")
dim(boot)
#> [1] 1000    6
```
...
``` r
ssm_trajectory(times = 0:4, draws = boot)
#> 
#> # SSM Trajectory:
#> 
#>  wave e_est e_lci e_uci x_est x_lci x_uci y_est y_lci y_uci a_est a_lci
#>     0  0.53  0.47  0.58  0.61  0.58  0.64 -0.10 -0.14 -0.07  0.62  0.58
#>     1  0.52  0.48  0.57  0.61  0.58  0.64 -0.04 -0.07 -0.01  0.61  0.58
#>     2  0.52  0.48  0.57  0.61  0.58  0.64  0.03  0.00  0.06  0.61  0.58
#>     3  0.52  0.47  0.57  0.61  0.58  0.64  0.09  0.06  0.12  0.61  0.58
#>     4  0.52  0.47  0.58  0.60  0.57  0.64  0.16  0.12  0.19  0.62  0.59
#>  a_uci  d_est  d_lci  d_uci
#>   0.65 350.28 346.84 353.47
#>   0.64 356.32 353.24 359.12
#>   0.64   2.47 359.58   5.32
#>   0.64   8.59   5.57  11.60
#>   0.66  14.53  11.17  17.81
...
The caution under this table is the one for the `draws` shape. The
intervals summarize the replicates as given, and the helper does not check
that they came from a joint fit. Here they did, because every refit used
the Section 4 model. On this sample the bootstrap medians of $x$ and $y$
sit close to the REML estimates of Section 5. The largest gap across the
five waves is 0.001.
The bootstrap matters more at samples smaller than these 150 persons,
where the REML intervals are expected to understate uncertainty most.

The loop needs an engine whose `simulate()` returns new responses for the
model. glmmTMB does. nlme's `simulate()` refuses a model with a `varIdent`
weight, and the Section 10 nlme call carries one. So a user on nlme runs
the bootstrap through the glmmTMB call of Section 4. The two engines fit the
same model, and their fixed effects agree within the bounds the package's
parity test holds.
...
glmmTMB table's estimates. The caution under the table is the one for the
`draws` shape: the intervals summarize the draws as given. Posterior draws
average over the variance components rather than condition on their
estimates, so the REML caution of Section 7 does not apply to them. A posterior interval from brms depends on its priors instead,
