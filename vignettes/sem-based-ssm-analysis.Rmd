...
general rule rejects an invariance step when CFI falls by more than .01. That
direction comes from the article's own Table 5, whose critical values are the
1% *lower* tails of the simulated null distributions, so a ΔCFI at or below one
is evidence *against* invariance; the sentence stating the rule on the article's
p. 251 reads backwards relative to its own table, and this package follows the
simulation. It is
...
estimated by ML whose CFI is the plain normal-theory one — `estimator = "ML"`.
Because the default estimator is `"MLR"`, the ladder above prints its `dcfi`
value marked as outside that validated scope, with no verdict attached, and the
note names the reason. The same withholding applies to a non-ML estimator such
as `"GLS"`, whose CFI is plain-named but whose fit function is not the one the
criterion was simulated under. That is a deliberate refusal rather than a gap:
extending the cutoff to a robust index, another estimator, or three or more
