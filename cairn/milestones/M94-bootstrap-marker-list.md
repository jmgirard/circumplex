...
- 2026-08-18: post-approval CI red (PR #123, first matrix run): ubuntu/windows/covr failed on exactly one test — the analytic byte-identity snapshot fence — via a third-decimal communality (0.562 vs 0.563) and the saturated free fit's residual tie-break pair; macOS (the capturing machine) passed, and every AC1/AC2 marker test passed on all three platforms. The fence is a same-machine claim (AC3: "regenerable at review from the merge-base commit"), so it now carries skip_on_ci/cran/covr like the print() fence, with the measured deltas recorded in its comment. Test-guard change only; the deliverable is untouched.
