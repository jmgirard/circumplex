...
- 2026-08-16: merge APPROVED at the round-3 gate for PR #117; marker written. PR marked ready (which re-triggered CI); the one blocking `gh pr checks --watch` timed out at 10m with pkgdown green and test-coverage + ubuntu-latest still pending. Nothing left watching; merge resumes from `gh pr checks` on demand.
