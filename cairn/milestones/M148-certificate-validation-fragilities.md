...
- AC6: `devtools::test(reporter = "check")` exit 0: FAIL 0, WARN 12, SKIP 1, PASS 13942. The twelve warnings all rise from files the branch does not touch (`git diff master..HEAD --name-only` lists only the certificate test file, the oracle driver, and tracking): four "the standard deviation is zero" from `test-pole-values.R:56` (a zero-variance profile by design), a glmmTMB/TMB version-mismatch warning and a lavaan version warning from the installed libraries, two "CPM Hessian is ill-conditioned" from `test-ci_accuracy.R`, and the rest of the same environmental kind. Zero failures holds; zero warnings does not, and did not before the branch was cut. FAIL as written at the first pass; superseded by the AC6 amendment (work log), re-read in the second pass below.
...
### Second pass (2026-09-22, after the AC6 amendment)

Branch at `85a1936f` plus the fix-now comment edits below; master `89989cba` unchanged, an ancestor of HEAD; no PR exists. Fresh evidence per criterion, on macOS arm64:

- AC1: plant `[-1L]` on the measured argument at the three `cert_pin_length()` sites, `NOT_CRAN=true`, all six cases priced: 18 failures, all 18 naming a measured-length site. Reverted. PASS.
- AC2: the AC7 harness test (19 expectations, 0 failures) ran in the clean file run under `NOT_CRAN=true`; the `-1.5` and `c(0.5, -2)` probes assert the "below -100%" message naming the input. PASS.
...
Independent review, second pass (three lenses, fresh context). All first-pass dispositions re-confirmed against the implementation. New findings, ranked as reported:
- O-2.1 (diff-bug): the M122 comment on `cert_rel()` (test file 280-281) still made the retracted claim that a NaN "passes an `expect_lt()` as NA"; verified false on testthat 3.3.2. **Fix now**: rewritten to say testthat reddens on NaN and the guard exists to name the zero.
- O-2.2 (diff-bug): two imprecisions in the new `cert_root_rel()` comment: the old failure did name the bracket's site, and the certificate's sentinel path does not stand in front of `cert_true_error()`'s own `dv`; the protection is that shipped variances are positive. **Fix now**: both clauses rewritten. The testthat claim is checked on 3.3.2 only; noted.
- O-2.3 (diff-bug): `exact_keys()` requires `EXACT_BASELINE`, which the Python script prints and the R driver never reads, while the comment said "every key this script reads". **Fix now**: the comment says the list is what the Python script prints, and why. The key stays required: a missing printed line means the output is not the expected one.
- O-2.4: O7's slack re-derived at about 25% headroom. Noted.
- S-blame-2 and S-prior-2: `cert_priced_not_degraded()`'s `identical()` against the sentinel re-confirmed as a reject with reason, and the sentence naming the M122 lineage the first pass owed is **fixed now** in the helper's comment.
- O4 (first pass, empty `HEX_*` value) re-confirmed as **follow-up**: a candidate row is written at hygiene under the RR22 follow-ons row (search-first: that row already holds the certificate-file remainders).

Return floor: no actioned finding fails a criterion or is load-bearing for users; none returns status.
