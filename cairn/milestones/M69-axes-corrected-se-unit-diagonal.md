...
- **AC4** — Citation reads `R/axes_corrected_se.R:171-179`: span 9 lines (<= 15), contains `diag(wc) <- -rowSums(wc * sigma)` and the unit-diagonal statement. Guard passes and is mutation-verified twice — a stale range fails 2 assertions, an over-wide range fails the span check.
...
### Review round 2 — evidence for the re-ticked criteria (2026-08-04)

- **AC1 re-verified.** Ledger extended with the two omissions round 1 found. `test-axes-corrected-se.R:42-45`: the asserted quantity moved +0.105% / +0.162% / +0.172% and now reads `fiml_ratio` rather than the mixed quotient, at a tolerance tightened 0.01 -> 0.001. Both regenerated fixtures now carry `regenerated: 2026-08-03 (M69)` and a `source` naming the repricing. Recorded caveat: the stamp lives in the committed `.rds`; a future re-run of `devel/m66-heavy-cells.R` writes the generator's own `source` literal again, so whoever regenerates next stamps their own milestone.
- **AC4 re-verified.** Both assertions now run over the parsed cited range, never the citing file at large, and the range `R/axes_corrected_se.R:171-179` states each side's pricing ("`corrected` and `fiml_ratio` are priced at cov2cor(Sigma-hat) while `naive` is priced raw"). Mutation-verified: stripping that sentence reddens 3 assertions. **The guard then caught a live rot on this milestone's own next commit** — the F1 comment block shifted the fold below the cited range and the guard went red, which is exactly the failure mode it was built for and the reason the citation was stale to begin with.
- **AC7 re-verified.** A second invariance test covers deviation D1's zeta2/blockwise probe: `corrected` and `fiml_ratio` invariant under a seeded positive diagonal and under scalar 2 at 1e-6, `naive` scaling by exactly 2, with `zeta2` present among the components.

Suites after the fixes: `test-axes-corrected-se.R` 25 tests / 792 assertions, `test-axes-scaled-fit.R` 27 / 641, zero failures, zero errors, zero skips.

### Review round 2 — findings (2026-08-04)

Three lenses re-run on the updated diff plus a fresh scorer. 18 candidates; **1 actioned (>= 80)**, 17 logged. **No acceptance criterion fails as written**, and nothing reaches the >= 90 return floor, so the milestone does not return.

**Actioned:**

- **A7 (80) — the zeta2 invariance test dropped the octant test's anti-triviality companion.** Without `expect_gt(max(abs(diag_rs$naive / base$naive - 1)), 1e-3)`, an edit that also normalized `naive` would leave every invariance assertion on that probe green while breaking the lavaan fence. **Fixed now.**

**Logged, not actioned** (17), the four worth naming:

- **A1 (48) — the F1 fix was not swept to its sibling.** `R/axes_scaled_fit.R:103` still reads `any(diag(sigma) <= 0)` and errors on NA/NaN exactly as `axes_corrected_se()` did before round 1; verified live. Scored out of scope because that guard predates M69 (M68 shipped it) and only the cross-reference in that file was in M69's scope. **Disposition: candidate row**, since M69 leaves the two surfaces RR15 B2 exists to make consistent behaving differently on identical input, and this is the third fix-one-miss-the-sibling instance this milestone recorded.
- **A9 (70) — this file's own AC4 evidence bullet carried a stale line range** (`:172-178`, span 7) against the shipped `:171-179`, span 9 — the exact class of citation rot AC4 was written to eliminate, inside the criterion's own evidence. **Corrected in place above** (round 1, corrected round 2); the code and guard were always right.
- **B3 (62) — the AC10 enumeration test still greps `return("...")` literals out of `axes_se_pricing()`** where BC5's procedure and the test's own title name the `na_out()` calls in `axes_corrected_se()`. Round 1 logged this as F14 and round 2 added commentary without fixing it; reported independently by two lenses this round.
- **B4 (58) — the FIML cells moved +0.125% and +0.034% against AC9's cited -0.062% cormat figure, in the opposite sign.** Explicable (RR15 B3: nothing cancels on the FIML path, fitted diagonal 0.943-1.072) and inside AC9's <1% bound either way, but the milestone never noted that RR15's common-mode argument does not survive on the one path the ratio is actually used.

Remaining logged: A3 (78) the NaN regression case asserts only `reason`; A4 (55) the F1 cases sit under a title asserting the opposite contract; A5 (55) the 0.001 fence clears the mixed alternative by 4.8% on xi1, not the ~2x implied, and RR13's 4-significant-figure anchors spend half the budget on their own rounding; A6 (55) the three AC4 phrase checks are one sentence, and widening the citation to `:170-179` would satisfy the `naive` grep from code; B8 (55) round 2 added 12 more lines inside an undocumented function; B2 (55) the oracle shares a positional convention with the code it checks; A8 (35) the fixture stamp is hand-applied and the generators would not reproduce it; B1 (30) the oracle computes `naive` and discards it though it is a free `fiml_ratio` fence; C1 (30) `<= 0` never catches `+Inf` and the comment implies otherwise -- verified to refuse correctly anyway on the real probe (`reason = "unidentified"`, all NA); A2 (20) `"singular"` mislabels a non-finite diagonal; B7 (20) the claimed reversed-range failure does not actually manifest; C2 (15) the repoint's coverage loss is already covered by AC2's oracle; B6 (25) contradicted by deviation D1, which assigns that assertion to AC7.

**Positive evidence the lenses added:** column-wise, `xi1` and `se_naive` are bit-identical to master in all three calibration cells and only `se` moved; in the bootstrap fixture `naive`, `boot` and `kept` are bit-identical and only `analytic` moved. Both regenerations are exactly as targeted.
