...
- [x] AC7: `Rscript -e 'devtools::test()'` reports 0 failures, and `Rscript -e 'devtools::check(args = "--no-manual")'` reports 0 errors, 0 warnings and no note that the merge base does not report. NEWS.md names the four pages and the page they replace.
...
- 2026-09-16: review gate chose "fix three, then merge". F1 fixed on page 1 (`.Rmd.orig` and `.Rmd`), and the matching Reworded pair in Decisions updated in place because the split guard reads it as its input. F11 fixed in `cairn/references/plain-vignettes.md`. F12 fixed in NEWS.md. P1 to P3 and F6 to F10 go to candidate rows at hygiene. F2 to F5 rejected with the reasons in Review.
- 2026-09-16: step-7 approval: m136-split-structure-vignette approved for merge.
...
- Reworded: When a boundary marker (defined in the next subsection) is present, the analytic caution prints up to very large $N$. => When a boundary marker (defined in "CPM Fits at a Boundary") is present, the analytic caution prints up to very large $N$.
...
- AC7: `devtools::test()` 0 failures, 11 warnings, 1 skip, 11283 pass, exit 0. `devtools::check(args = "--no-manual")` 0 errors, 0 warnings, 0 notes, the same as master at the M135 review. NEWS.md names the four pages and the page they replace (lines 110-119). Ticked.
