...
- [x] T9: Fix the two ledger guards in `test-print-width.R`: check entries
...
- [x] T10: Validate `wrap_prose()`'s `width` and `atomic` arguments as it
...
- 2026-09-15: T9 done (review findings O5, O6). Guard two no longer depends on the 41 fixture blocks having run first: when the gathered domain is empty it rebuilds the domain itself and says which it used. Verified by making the blocks record nothing, where the guard still passes; before the fix that run failed on an empty domain for a reason unrelated to the ledger. O5 cannot be closed by a textual rule, and the reasoning is written into the file's header: an unwrapped caution never reaches `cat_prose()`, so instrumenting the helper does not see it, and T3 already rejected a shape pattern. What stands in for it is classification — every ledger entry is now named by its kind, and a third guard requires that kind to be one of the four the Scope leaves alone. Parking a caution now means writing a false kind into the diff. Probes: a caution parked under `heading` reddens two guards; an entry with no kind reddens the new one. 44 blocks, 631 assertions, 0 failures.
- 2026-09-15: T10 done (review findings O8, O9). `wrap_prose()` and `cat_prose()` now take `width = NULL`, meaning "read the reader's console", and validate any width the caller states with `stopifnot(is_scalar_count(width))` instead of repairing it to 80. `atomic` must be `TRUE` or `FALSE`, never `NA`. The measurement that shaped this: R refuses `options(width = NULL)` ("option 'width' cannot be deleted") and `options(width = -1)` ("allowed 10...10000"), so `getOption("width")` is always a usable integer and the old fallback was a branch no test could reach. It was removed rather than kept untested, and a test now pins R's guarantee as the reason. The pre-existing fallback test was rewritten to the new contract.
- 2026-09-15: T10, O9. `m94_expect_caveat_indent()` restores a direct pin on the fired-marker caveat's two-space continuation leader, at both assertion sites in `test-cpm_summary_markers.R`. It finds the caveat's end by consuming lines until they carry all of its words, because the closing phrase straddles a line break at most widths and a raw match finds nothing. Probe: emitting the caveat with `continuation = ""` reddens both blocks.
- 2026-09-15: T10 used base R `options()` with `on.exit()` rather than `withr::with_options()`. `withr` is in no DESCRIPTION field, and adding it would be a dependency change needing its own gate and D-entry for a test convenience.
- 2026-09-16: claim audit: 34 claims read, 7 corrected — R/utils.R, tests/testthat/test-wrap-prose.R, tests/testthat/test-print-width.R, tests/testthat/test-cpm_summary_markers.R, tools/m131-line-identity.R
