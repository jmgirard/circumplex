...
- 2026-08-22: T9 — findings 1, 2, 6, 7, 9, 10, 11 fixed together, since all seven are the one diagnostic. `axes_degeneracy_hint()` no longer reads an eigenvector: a pair qualifies when `|r| >= 1 - λmax·sqrt(p·ε/τ)`, which by Cauchy interlacing forces λmin at or below the criterion's own floor by itself. That is basis-free by construction (F2 — measured on one p = 16 matrix with eight duplicate pairs, relabelling alone moved the old message between naming an arbitrary member pair and naming nothing), and it replaces both hand-set constants with one read off the criterion (F11). Several qualifying pairs now give the count and plural advice; "drop one" was wrong counsel among equals. The hint is attached only to the `"ill_conditioned"` literal at both call sites (F1: an indefinite refusal was printing a negative condition number), and NEWS is scoped to the criterion's refusal, leaving the cval backstop's same-literal door explicitly undiagnosed (F5/F6). The r format states a bound past what `%.6g` can separate from 1 (F7), and the two false comment rationales are gone — `cormat` does require `items` (F9), and both call sites realign to `item_names` before the hint runs, so dimnames were never the risk the comment claimed (F10).
- 2026-08-22: T9 — five mutants, each red at a different pin family and each restored by copy with the source blob re-hashed to 3f193efe / 49dad4bd after every one: ungating the scaled-fit call site reddens the F1 pin alone; a flat 0.99 cut reddens the F11 pin alone; a fixed-width `%.6g` reddens the F7 pin (2 assertions); reversing the pair list's order reddens F2's listing assertion; naming only the strongest pair reddens F2 at 10 assertions. The F11 case is what makes the derived cut load-bearing rather than decorative: a matrix whose λmin is driven by a ten-item alternating combination while one pair sits at r = 0.995 — past 0.99, short of this p's 0.99997 cut — so a hand-set 0.99 names a pair that interlacing bounds λmin only by 5e-3, twenty-five times above the floor.
- 2026-08-22: T9 — finding 4 fixed at Jeff's gate choice: `exact_oracle.R` computes `df` and `baseline_df` per case from that case's own p and parameter count instead of handing every case the p = 3 fixture's 1 and 3. Nothing reads `EXACT_CVAL` outside the fixture today, so no measurement moves; the anchors and both windows still pass.
- 2026-08-22: T9 — finding 14 fixed (AC4 case 2 now asserts `details$converged`, so the criterion's literal cannot be reached by a non-convergence route unnoticed) and finding 13 fixed properly rather than by relabelling: the AC5 diffuse probe is `cov2cor`'d, so the entries the threshold is stated in genuinely are correlations.
- 2026-08-22: T9 — the ubuntu-only CI failure repaired as the record planned. AC4 case 3's carrier fit is now `probe_octant()` (p = 24, κ = 10.45) rather than `m106_family_a(1e-2, per_scale = 3L)` at κ = 721; the injected `r24_bad` is unchanged but relabelled to the carrier's item names, which the downstream realignment requires. `devtools::test()` FAIL 0 / PASS 8477 / SKIP 3 / WARN 5 — the same five pre-existing warnings, count unmoved.
- 2026-08-22: T9 — `withr::with_seed()` reached for in a first draft of the F2 pin and removed before commit: `R CMD check` WARNed "'::' or ':::' import not declared from: 'withr'", and withr is not in Suggests (two other test files already record the same convention). Acquiring a dependency for a test is a gated decision, and none was needed — the claim is about every relabelling, so five fixed permutations cover it better than seeded draws, the evens-then-odds one splitting every duplicate pair across the halves.
...
- 2026-08-23: review gate — status review→in-progress, defect return 2 of this milestone. What failed: AC5 on round-2 finding 1 (gating the diagnostic on the `"ill_conditioned"` literal does not exclude a negative smallest eigenvalue, because M90 files roundoff-level negativity under that same literal — so the negative condition number round 1 returned on is still reachable, measured at −7.75e+07 / −3.87e+07 / −2.74e+07 on the very spectral form AC3 enumerates, and `Inf` on an exactly collinear pair; the round-1 repair narrowed the symptom rather than closing it); and AC1 on finding 3 (the derivation's load-bearing step — that the corrected SE's relative sampling SD matches a single variance's despite being a functional of all p(p+1)/2 entries of Σ̂ — is asserted rather than argued, and the premise is never stated, against a criterion that requires every premise stated). Findings 2, 4, 5, 6, 7, 9, 10, 11, 13, 14, 15, 17, 20 ride along: two further measured claims wrong inside AC1's own block, one test gap, the rest comment and NEWS corrections.
