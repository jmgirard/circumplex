...
- 2026-08-14: merge approved by the user at the review gate for PR #114; marker written. CI not green at approval time — `ubuntu-latest (release)` and `test-coverage` still pending after two blocking waits timed out (pkgdown passed at 4m22s). Both watches stopped with nothing left watching; the merge waits on a fresh `gh pr checks` read in the next session, per the stateless-resume rule.
