...
- [ ] T6: relax the ≥ 2-items refusal to the drop rule; keep an informative error for anything still unsupported.
- [ ] T7: bank the six COC/SYMLOG rows in `cairn/references/strack2013.md`; add the ±.01 formula-layer sweep plus the AC4 end-to-end fractional-item_n fence (odd-k single-item and mixed unequal-count fixtures).
- [ ] T8: Layer-B cells at a single-item configuration, incl. a k = 5 cell (item_n 2.5) — population Σ with no ζ1 term (`axes_population_cor()` at `n_items = 1` puts the ζ1 block on the diagonal, so `zeta1 = 0` suffices), synthetic recovery, and OpenMx without its `zeta1*B` term (`test-:247-315`).
- [ ] T9: roxygen (incl. the two-dimensional scope sentence), `man/`, vignette, NEWS; fix the two SYMLOG-shape comment mislabels (`:73-75`, `test-:1166-1168`); full check plus the PDF manual.
