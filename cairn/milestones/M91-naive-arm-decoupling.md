...
- **Independent review (three-lens fan-out, user-facing tier).** [S] blame-history: no findings (verified the anti-relabelling invariant, M90 arm order, M71 warning contract; every test re-expectation strengthened prior pins). [S] prior-review: no findings (checked M62/M66/M69/M70/M71/M89/M90 archives + LESSONS; PR-comment probe empty, walk skipped). [O] diff-bug: core logic confirmed correct (same-matrix fiml_ratio coherence, no leak path either direction); 9 ranked findings, none floor-qualifying. Dispositions:
  - F1 (roxygen/NEWS/comment "the two surfaces' user-facing refusals agree exactly" is an overclaim — pricing backstops and df doors are surface-local) — **fixed**: claim scoped to the shared criterion in all three homes, surface-local refusals named.
  - F2 (`@return` documented `naive_reason` causes narrower than code: raw-arm *pricing* failures also set it) — **fixed**: doc now names both routes and the pricing literals.
  - F3 (`na_out()` hardcodes `naive_reason = NULL`, silently discarding a determined raw-arm refusal) — **fixed structurally** via F5's reorder: every `na_out()` exit now precedes the raw arm, so no determined refusal exists to discard; deliberateness comment added.
  - F4 (details comment "only the correlation-metric arm sets this field" ignores the metric-blind doors) — **fixed**: wording now names them.
  - F5 (raw arm priced before cov2cor arm: wasted 24×24 sandwich on unit-refusal inputs) — **fixed**: cov2cor arm (criterion + pricing) resolves first.
  - F6 (AC10 test title asserts the retired every-non-success contract) — **fixed**: retitled to the reported-vector contract.
  - F7 (M91-D2 rescope spliced mid-sentence into the band rationale) — **fixed**: rescope moved to its own sentence at the end of the rationale.
  - F8 (duplicate all-NA vector construction) — **fixed**: `empty` hoisted, one definition.
  - F9a (renamed assembly test leaves M89 AC2 ungreppable in the test file) — **fixed**: lineage named in the test comment.
  - F9b (AC1's 1e-9 pin asserted at the helper while T1's task prose said "end-to-end") — **rejected**: AC1's text names no venue and is satisfied as written by the helper test; the venue split is task prose, recorded in the T1 work-log line at implementation, and AC5 carries the through-assembly assertion. Not a criterion reinterpretation.
  - Post-fix: `document()` no diff beyond regenerated Rd, zero link warnings; Wc citation range re-verified still landing (all fix edits fall outside 176-184); full suite FAIL 0 / PASS 8300.
