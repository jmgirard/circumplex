...
- 2026-07-26: merge approved by Jeff at the gate (marker written for PR #90). PR marked ready; one blocking `gh pr checks 90 --watch` timed out at 600s with pkgdown **pass** and `test-coverage` + `ubuntu-latest (release)` still pending, so per the CI rule the fresh state is recorded here and nothing is left watching. Merge is deferred to a resumed pass: the approval marker is single-use but survives an unattempted merge, and status stays `review` until the squash lands.
