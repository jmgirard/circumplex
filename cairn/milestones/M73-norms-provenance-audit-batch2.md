...
- [x] AC1. Each of iis32, iis64, ipipipc and isc has a committed
...
- [x] AC2. With `AUDIT_BATCH` extended to the nine audited instruments,
...
- [x] AC3. Every ledger row for the four new instruments whose `kind` is
...
- [x] AC4. `stamp_ledger()`, extracted from `audit-norms.R`'s inline run block,
...
- [x] AC5. Every `Reference` row in the nine notes' audit-values blocks carries
...
- [x] AC6. `norms-audit.md` records a verified verdict for each of the four
      ...
      red.
- [x] AC7. Two record corrections. (a) `grep -rn -i "in press" R/ man/ NEWS.md
...
- [x] AC8. `devtools::check(args = "--no-manual")` is clean (0 errors, 0
...
- 2026-08-07: review round 3 — all eight criteria verified on the round-3 tree; consistency gate clean. Three lenses: blame-history no findings, prior-review no regression, diff-bug 18. Two scored >= 80, neither meeting the return floor, both fixed at the gate: the 16 ipipipc disposition reasons still saying p. 353, and `?iis32`'s unsourced list omitting the sample description. Sixteen logged below threshold. No return.
- 2026-08-07: round-3 verify slot clean. `devtools::test()` 0 failures over 5986 assertions (4 warnings, all in `test-ci_accuracy.R`, which this branch does not touch); `devtools::check(args = "--no-manual")` 0 errors, 0 warnings, 0 notes (24m41s); PDF manual built separately with `R CMD Rd2pdf --no-preview --force` (exit 0, 363 KB) per the M57/M7 lesson, its pdfTeX warnings the same pre-existing cross-package hyperlink destinations (ggplot2, lavaan, boot, base) round 2 recorded, none from `ipipipc.Rd`. Status back to review.
...
`master` had not moved since round 2, so no re-merge was needed and round 2's
tree is the base. Every criterion re-executed on the round-3 tree.

- AC1 **verified.** `parse_source_note()` reads all four notes (38/36/37/38
  rows), each with exactly one `Reference` row. Each Provenance block's
  extraction status, read through `cairn_validate`'s own `_extraction_status`
  parser, carries a verification claim, names the channel that read the anchor,
  and ends in an `— observed` stamp; all four have INDEX.md lines.
- AC2 **verified.** Audit re-run: 130 ledger rows, 13 coverage rows, 0
  coverage gaps, 0 angle-copy splits, 0 IP2 breaches, 0 `UNDISPOSITIONED`.
- AC3 **verified.** Exactly three `mismatch` rows across the four new
  instruments — iis64, ipipipc and isc `Population` — each carrying
  `intended-deviation`. No shipped norm value needed correcting.
- AC4 **verified.** `stamp_ledger()` returns 0 and 1 rows without error and
  the same ten column names for both.
- AC5 **verified.** All nine `Reference` rows re-read against the sources.
  Eight quote a page anchor; the ninth (locke2000) carries the
  `constructed-credit` token with its reason, and `audit_norms()` emits it as
  the one exempt coverage row (`csiv | constructed-credit-reference |
  Reference`). The three round-2 failures are closed at their root: renders of
  bliton2019 p. 1 and locke2014 p. 433 show no running head, and both notes now
  say so outright; locke2000's reason is narrowed to the undergraduate table;
  the en dashes and superscript affiliation markers match the renders. The
  independent Opus lens re-derived every one of these against the source PDFs
  and confirmed them.
- AC6 **verified.** The four verdicts were recomputed from the shipped and
  note-side values rather than read: what actually compared and passed is
  iis32 {Items, Reference, URL}, iis64 {Items, M, SD, Size, Reference, URL},
  ipipipc {Items, Size, Reference, URL}, isc {M, SD, Size, Reference, URL} —
  each verdict names exactly that set and disposes the remainder correctly.
  Both mutation directions reproduced red: dropping isc from `audited_objects`
  fails with `Needs: "isc"`, flipping its status row to `unaudited` fails with
  `Absent: "isc"`.
- AC7 **verified.** (a) The named grep returns one NEWS.md hit, the sentence
  describing this fix, which credits neither author set. Both `@source`
  entries give published citations and `man/` regenerates with no diff.
  (b) The iip32/iip64 rows no longer claim an unobtainable source.
- AC8 **verified.** `check(args = "--no-manual")` 0 errors, 0 warnings,
  0 notes (19m30s on the reviewed tree; re-run clean after the two findings
  fixed at this gate). PDF manual built separately with `R CMD Rd2pdf`.

### Consistency gate

`cairn_validate` all checks pass, exit 0 (48 advisories: 47 pre-existing M7
work-log format lines plus the standing M73 sizing tripwire, 8 acceptance
criteria against the 7 threshold). `document()` no diff; `check_pkgdown()` no
problems; NEWS carries the user-visible entries; the only top-level file the
branch touches is NEWS.md, already present; README untouched. No DESIGN
principle changed, so `cairn_impact` is skipped.

### Findings

Three fresh-context lenses. The blame-history lens returned **no findings**,
confirming the CRLF endings held, the iei hotfix is undisturbed, and the
p. 354 correction propagated to every artifact it names. The prior-review
lens returned **no regression**, its GitHub probe again `[]`, with one
borderline point offered for scoring. The diff-bug lens returned 18. Scored by
an independent Sonnet scorer against the diff and the milestone file.

Actioned (>= 80), both **fixed at this gate**:

- **F-1 (85). Stale `p. 353` survives in the dispositions CSV.** All 16
  ipipipc `source-not-identified` reasons in
  `data-raw/norms-audit-dispositions.csv` still read "Study 1's combined
  sample (p. 353)"; round 3's page correction reached the notes, INDEX.md and
  the roxygen but missed this file. Corrected to p. 354.
- **F-5 (82). `?iis32` omits `Population` from its unsourced list.**
  `R/instrument_data.R` named only the means, standard deviations and the N of
  1380, while `hatcher2012.md`, the dispositions CSV and `norms-audit.md`'s
  iis32 verdict all treat `Population` as equally unsourced. The user-facing
  roxygen now names the sample description too; `man/iis32.Rd` regenerated.

Neither meets the return floor. F-1's closest criterion is AC3, whose named
procedure enumerates `mismatch` rows; the stale text sits in
`source-not-identified` dispositions on `not-published` rows, outside that
domain. F-5 names no criterion and scores below the 90 bar for a
deliverable-facing defect. Both were cheap and are fixed rather than deferred.

Logged below threshold (17), one line each: F-10 (65) locke2000's
constructed-credit reason says no printed text supplies the credit's author or
year, and the page's first-person "the 64-item version of the CSIV I
administered to 1,200 University of Idaho undergraduates" plus its "© 2024
Kenneth Locke" footer arguably support the *author* half — genuinely arguable
and left as written; F-8 (55) markey2009 swaps its Study 1 sample labels
between p. 354's prose and Table 1, unrecorded where hatcher2009's analogous
684-vs-686 got a headed section; F-15 (55) `norms-audit.md`'s per-row
`observed` dates were not all bumped for the round-2/3 content fixes; F-17
(50) markey2009.md's enumeration of Study 2's Results is not exhaustive though
its load-bearing "no octant M or SD" claim is correct; F-6 (50) the
containment rule lets a disclaiming `Reference` register as verified (G17/F17
recurrence); F-3 (45) `?iei`'s Crossref citation is unanchored on the shelf
and the shipped credit still says 2024 (F8/F9 recurrence, iei is M74 scope);
F-2 (35) the ledger's commit stamps necessarily precede the commit that lands
them (F6/G9 recurrence, generation-order behaviour); F-4 (35) `?iitc`'s online
date is likewise Crossref-derived, deliberately left; F-7 (35) the ledger
records no `match` rows, so positive verification leaves no machine-readable
trace (pre-existing from M72); F-13 (35) the constructed-credit detector is an
unvalidated substring match with no test (F13/G13 recurrence); F-14 (35)
nothing machine-checks AC5, whose verification method is manual by design;
F-9 (30) hatcher2012's 496-vs-497 clinical N unrecorded (F22/G23 recurrence);
F-12 (30) the Hatcher notes' `Items` rows share an origin, disclosed in prose
and outside AC5's `Reference`-only scope; F-11 (20) locke2000's `Size` anchor
quotes "N=1,200" where the page prints "1,200" in prose — pre-existing and on
an unmodified line; F-16 (20) the boudreaux2018 en-dash spacing claim is
arguable either way; F-19 (20) hatcher2012's small-caps byline, self-refuted by
the lens that raised it; F-18 (10) one unwrapped long line.

### Disposition

All eight criteria verified, the consistency gate clean, and the two actioned
findings fixed on the branch. No return.
