...
- 2026-09-01: gated amendment (/milestone-plan) — Jeff declared the release window open; `Depends on:` gains **M119** (pre-submission re-verification after the 302 post-merge commits: CRAN-mode test time 550 s vs 242 s in July, unguarded vdiffr sites, stale NEWS/cran-comments dependency and vignette claims). Status stays `blocked` until M119 is done; then `/cairn-release` runs the walk and T4 hands off. Today's re-verification: `check(manual = TRUE)` and CRAN-mode `R CMD check --as-cran` both 0/0/0 on HEAD 28142f36; no reverse dependencies; urlchecker clean (the SAGE-403 note did not reproduce).
...
- 2026-09-01: /cairn-release ran the walk on master d9b3212f (package content 5c4b1ae1). Version confirmed 2.0.0 at the gate (three-plus documented behavior changes to 1.2.0 surface); NEWS.md already consolidated under `# circumplex 2.0.0`, breaking changes first, no internal identifiers. Verified: `devtools::check(manual = TRUE)` Status OK 0/0/0 with PDF manual OK and vignette rebuild `[50s/51s]` OK, tests `[11m/11m]`; `devtools::document()` no diff, 0 unresolved-link lines; `pkgdown::check_pkgdown()` no problems; `build_readme()` left README.md unchanged (only the non-deterministic plot PNG, reverted); `urlchecker::url_check()` all 28 URLs correct — the SAGE DOI 403 is gone, so cran-comments' URL note now records the clean result; reverse dependencies none (`available.packages()` against cloud.r-project.org); master push CI green on all five configurations. cran-comments.md updated: Darwin 25.6.0, the as-cran run added, URL note rewritten, and the behavior-change list corrected from three to six — `norm_standardize()` and `cais` both shipped in 1.2.0, so the withdrawn CAIS sample, the out-of-range refusal, and the `ssm_plot_circle()` warning are changes to existing surface. win-builder R-devel uploaded 2026-09-01 21:05 CDT, results to me@jmgirard.com; only Jeff can read them, so confirming them clean gates submission. Blocker is now the submission handoff (T4), not M119.
...
- 2026-09-02: /cairn-release re-ran the walk on master c676355c for the resubmission, after M120 merged. Jeff declared the release window; version stays 2.0.0 (the pretest rejection published nothing). Verified at this tip: `devtools::check(manual = TRUE)` Status OK 0/0/0 with the PDF manual and the full suite `[10m/10m]`; `R CMD check --as-cran` on the built tarball with `NOT_CRAN` unset, idle machine, 110 s total — tests 23 s, vignette re-build 6 s, install 11 s, examples 10 s plus donttest 14 s, installed size 5.2 Mb (INFO, not a NOTE); `document()` no diff and no unresolved-link warning; `pkgdown::check_pkgdown()` no problems; `urlchecker::url_check()` all 28 correct; `build_readme()` left README unchanged; no reverse dependencies; master CI green on all five configurations for 25e80830. A fourth check run is discarded as contended — another project's check ran on this machine and inflated the donttest step to 174 s elapsed against an unchanged 14 s of CPU. cran-comments' local total corrected 1m47s → 1m50s. The win-builder run it cites was on 321cf648, two timing-neutral files behind this tip (a dead test helper removed, volatile-number markers added to one precomputed vignette), so at the gate Jeff chose to re-run `check_win_devel()` on the exact tarball before submitting. Blocker: that emailed win-builder result, then T4's `submit_cran()`.
- 2026-09-02: win-builder re-run on the exact resubmission tarball, read from the live artifacts at https://win-builder.r-project.org/K5OdLiXz416v (Jeff ran it; results reach only him). R-devel r90457 ucrt, Windows Server 2022, `Status: OK`, no NOTE/WARNING/ERROR, and the string `checktime` absent. Timed steps: feasibility 14s, R code 42s, examples 38s, tests 103s, vignette re-build 16s, PDF manual 24s = 237s; start stamp 2026-09-02 20:49:59 UTC against the 22:56 listing time (UTC+2, the only offset consistent with the timed sum) puts the run at 6m01s-7m00s. That is 1.5x the 321cf648 run's 158s timed sum on near-identical content, so win-builder load varies by more than the CRAN/win-builder machine difference does. Scaling this run by the 1.05 per-step CRAN/win-builder factor and adding CRAN's own 206s untimed residual projects ~7.6 min on the pretest machine against the 10-minute rule. cran-comments' win-builder sentence replaced: the "under 5 minutes" claim (true of 321cf648, not of this tarball) gives way to the tests/vignette figures win-builder itself prints, against the same two steps' pre-change 15m/146s.
- 2026-09-02: T4 handoff done again — Jeff ran `devtools::submit_cran()` at 21:11:08 UTC on SHA ad475b65, recorded in `CRAN-SUBMISSION`. That SHA is the tip every check in this walk was run against, so the submitted tarball carries the corrected cran-comments (the 2026-09-01 submission did not: its trim landed two minutes after the tarball was built). Awaiting the CRAN confirmation email and the incoming-pretest result. M7 stays `blocked` on that outcome; AC4 is not ticked and the milestone is not `done` until the release is accepted, per the 2026-07-19 status entry.
- 2026-09-02: CRAN's auto-check service returned "on its way to CRAN" — the incoming pretests passed, so the 24-minute Windows check-time NOTE that rejected the 2026-09-01 submission did not recur. Queue position read directly: `circumplex_2.0.0.tar.gz` sits in https://cran.r-project.org/incoming/publish/, the approved-for-publication folder, not `inspect/`, `pending/` or `recheck/`. The CRAN package page still shows 1.2.0 (published 2026-07-02) — observed 2026-09-02 — so publication has not propagated yet. M7 stays `blocked` until the page shows 2.0.0; nothing is tagged before then.
