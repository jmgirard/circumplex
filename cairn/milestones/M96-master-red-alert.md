...
- [x] T10: Build AC3(b) in `tools/master-red-alert-dryrun.R` — capture the
...
- 2026-08-18: T10 — AC3(b) built: the dry run now captures the `--title`/`--body` of every recorded `gh issue create` and `gh issue comment` in every fixture, replaces each synthetic payload value with its field name, and requires the result to equal a committed template (guarded by a precondition that the four values are non-empty and substrings neither of one another nor of the template). It catches all four composition routes the second return found — at the create call site, on the comment call, on the unlabeled fallback create, and `BODY` re-composed after its heredoc — each of which the deleted scanner passed. Also fixed F5: `gh label list` and `gh issue list` failures no longer abort the alert, with a fifth fixture (`label list refused`) pinning that it still posts unlabeled; the header now states what the harness does not decide (flag-blind stub, so F4's dropped `--label` is not caught; local-vs-runner expansion).
- 2026-08-18: gate fixes (third-pass [O] review, F1/F2/F3) — AC3(b)'s capture no longer uses in-band framing: the stub writes each call's argv to its own NUL-separated file, so a body line beginning `== ` can no longer forge a block boundary and truncate its own capture (verified: a forged header plus `runner: $(hostname)` passed before, fails now). The normalization gained an injectivity guard — a capture already containing a field marker is refused, so hardcoding `<name>` in the table no longer reduces to the template. And the audit now refuses a step-level `if:`, which would silence the alert entirely while the job-level gate AC1 pins stayed intact. Full mutation battery re-run: 15 of 15 caught, including every mutation from the two earlier passes.
...
**Third-pass fan-out.** One fresh-context [O] diff-bug reviewer (the descope is bounded and the earlier lenses had already reported on the surrounding history). Ten findings.

- **F1 (fixed at the gate) — "AC3(b)'s capture is delimited by an in-band header that the issue body itself can forge."** Verified: a body line `== label list` followed by `runner: $(hostname) token-prefix: ${GH_TOKEN:0:6}` left both scripts at exit 0 with all five fixtures "ok", while the text handed to `gh` genuinely carried the composed content, including a slice of the token. Fixed by removing in-band framing entirely — one file per call, NUL-separated fields.
- **F2 (fixed at the gate) — "AC3(b)'s normalization has no injectivity guard on the field-name markers, so a hardcoded placeholder passes."** Verified: rewriting the table rows to literal `<name>` / `<conclusion>` passed, because the presence check was satisfied by those values' other occurrences. Fixed: a capture already containing a marker is refused.
- **F3 (fixed at the gate) — "a step-level `if:` disables the whole alert and both scripts pass."** Verified. Outside AC1's wording, which names the job's `if:`; fixed anyway, as a one-line assertion against a defect that silences the deliverable.
- **F5 (accepted, documented) — a transient `gh issue list` failure degrades to "no match" and opens a duplicate labeled issue.** Deliberate: posting beats silence, and this is the same trade the label fallback makes. The consequence is now named in the body's comment rather than left implicit.
- **F6 (won't-fix) — `gh issue list` has no `--limit`, so it defaults to 30.** The labeled open set is held near one or two issues by the dedupe itself; raising it is a fix without a failure mode to prevent.
- **F8 (fixed) — the Coverage line for AC3 omitted T10**, which builds AC3(b) entirely. Remapped to `AC3 → T9, T10`.
- **F4 (won't-fix) — the workflow file still advertised the deleted scanner.** Corrected: the comment now says what actually checks it.
- **F7, F9, F10 (won't-fix) — `cancel-in-progress: false` cancels a pending run rather than sparing it (restates second-pass F7); AC5's ordering grep matches comment lines (safe in the pass direction); `--body`-not-last would fail with a misleading message (fail direction only).** All accepted at internal tier under the descope.
- Restatements of what the descope no longer promises (not defects, per the reviewer's own classification): a dropped `--label`, local-vs-runner expansion, a deleted `set -euo pipefail`, `startup_failure`/`timed_out` silence, and the `case`-vs-`[[ ]]` comment.
