...
- 2026-09-05: post-fix re-verification — `devtools::check(manual = TRUE)` on the fixed branch: `Status: OK`, 0/0/0, 22m 15.7s, PDF-manual step included; a fresh `R CMD build` tarball lists 357 paths, 0 under `tools/`, 0 matching `arm64` or `Dockerfile`. AC6 holds after the `.Rbuildignore` change. AC1 and AC3 were re-run on the fixed `check.sh` and still discriminate (exit 1 / `Status: 1 ERROR` and exit 0 / `Status: OK`).
- 2026-09-05: step-7 approval: PR #154 approved for merge.
- 2026-09-05: CI wait hit the harness ceiling twice on PR #154 with `ubuntu-latest (release)` still pending (`matrix`, `pkgdown` and `vignette-precompute` all pass); the moved watcher was stopped, nothing is armed, and the merge was not made. Approval stands; re-enter with `/milestone-review M121`, which re-derives the check and merge state.
