...
- [x] AC7: The verify slot is clean. `devtools::document()` produces no diff. NEWS.md names the export. `_pkgdown.yml` lists it. The help-page examples use a small inline `coef` and `vcov` and run without glmmTMB installed.
...
- 2026-09-24: /milestone-review: seven criteria verified, three-lens review, seven fix-now findings landed on the branch (140bf794), one candidate row (O8), four rejected.
- 2026-09-24: step-7 approval: m152-growth-trajectory-helper approved for merge.
...
- AC7: `devtools::check()` on the tree after the fix-now commits (140bf794): `Status: OK`, 0 errors, 0 warnings, 0 notes, tests OK, 7 min. `document()` no diff and no link warning. NEWS.md names `ssm_trajectory()`. `_pkgdown.yml` lists it under Growth Model Functions. The examples in `man/ssm_trajectory.Rd` call base R and the package only; sourced under `load_all()` they ran with glmmTMB never loaded.
...
**Independent review**, three lenses, user-facing tier. Findings ranked as reported, with the proposed disposition (confirmed or changed at the gate):
- O1 (diff-bug): the AC3 straddle test defined "straddles" as `d_lci > d_uci` and checked only those rows, so a bound stored at -0.5 would pass. Fix now: all three `d` columns asserted in [0, 360), and the straddling waves decided by the delta-method direction interval on the signed branch (wave 2 only, margins over 0.4 degrees), compared to the storage predicate.
- O2 (diff-bug): a `vcov` that is not positive semidefinite drew silently, since `mvn_root()` clamps negative eigenvalues. Fix now: refused when the smallest eigenvalue is under -1e-8 times the largest, with a test; the fixture (smallest eigenvalue 1.3e-5) passes.
- O3 (diff-bug): `draws` accepted `Inf`. Fix now: refused with `NA`, with a test.
- O4 (diff-bug): row names on a `contrast` result were ignored, so rows named `x, e, y` swapped estimates silently. Fix now: named rows must be `e, x, y` in that order; unnamed rows are taken in order; both tested.
- O5 (diff-bug): the all-zero exemption read the whole `vcov`, so a zero six-name block beside a nonzero covariate was refused. Fix now: the exemption reads the block the contrast uses, with a test. The second case, a `vcov` nonzero only for `e`, is still refused with the joint-fit message; rejected, since AC4 as written binds it and `x` and `y` then have no variance to fit.
- O6 (diff-bug): with a single time only the covariance terms that time reaches are read, so a joint fit can be refused at `times = 0`. Not a violation; the help page now says the check reads only the given `times`.
- O7 (diff-bug) and S1 (prior-review, M35 finding 76): an `NA` in `certified` printed as certified. Fix now: an `NA` row is marked uncertified, with a test.
- O8 (diff-bug): the caution names a fitted model's fixed-effect covariance, wrong for draws input. Follow-up: a candidate row, since AC6 binds the line as unconditional and M153 rewrites the page that explains it.
- O9 (diff-bug): the linear-interval tolerance was relative. Fix now: absolute 0.01 as AC3 states, with the measured power (0.017 shift from one dropped slope covariance) in the test comment.
- O10 (diff-bug): the column-subset print test cannot fail on the mark logic. Rejected: it guards the crash it was written for.
- S2 (blame-history): AC7 unticked while status is review. Rejected: the box is ticked only against recorded evidence, and the full check was still running.
- S3 (prior-review): the secondary surface probe returned no PR review comments; M35 finding 94 (a `time` name clobbering a value column) and the M26 explicit `type` rule are both honored.
