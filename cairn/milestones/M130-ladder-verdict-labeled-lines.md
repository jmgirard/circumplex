...
Second pass, 2026-09-15, at `eb1b6f0d`. `origin/master` is `e68dd8ed`, an ancestor of the branch, so no merge was needed. Fresh records of all 14 `helper-ssm-sem-ladder.R` cases were taken at `0f8f3289` (a scratch worktree) and at the head with one script, the ladder block kept at widths 77 and 80.

Evidence:
- AC1: the head prints a `Verdict:` line and labeled lines in all 14 cases. The configural case prints `Test: none run`. The vacuous cases say the metric rung holds by construction and is not testable. vacuous_above and retained_above add an `Also:` line that marks the scalar rung as reported only. retained_two prints one `Test:` line per rung. With a requested contrast, a non-comparable case prints `Contrast:`, `Profiles:` and `Instead:`. Without one, rejected_plain and untestable_plain print that a latent contrast is not computable, and `Profiles:`. A long value continues on an indented line under its label. The M130 test "for every verdict arm" asserts the labels and values of 12 cases, which include every named AC1 case.
- AC2: the `dcfi` and `cr` columns and the note print in the 5 cases where `dcfi_scope$in_scope` is TRUE and a `dcfi` value is not NA (vacuous_above, both rejected cases, both untestable cases). They print in none of the other 9. In all 5, the note is 2 lines at widths 77 and 80. The note names Cheung & Rensvold (2002), the two-group ML simulation, alpha = .01 and the -0.01 rule. It also says "the verdict does not use it". The out-of-scope cases include robust CFI (retained_one), GLS (gls), 3 groups (groups3_ml) and a combination (groups3_mlr). In all 14 cases, `res$invariance` is `identical()` to master. The M130 test "dcfi, cr and a Delta-CFI note of at most two lines print only inside the criterion's scope" covers these cases.
- AC3: in all 14 cases, `res$invariance`, the warnings, the `ssm_plot_contrast()` errors and the stored verdicts are `identical()` at `0f8f3289` and `eb1b6f0d`. The master and head blocks at width 80 were read side by side for every case. Each table value, rung note, test statistic, decision and caution in master's block prints at the head, or it is out-of-scope ΔCFI text that AC2 removes. The wording changes match the T5 and T14 ledgers. For untestable_plain, "not computable on this instrument's latent metric" becomes "not computable, because comparability cannot be established", by maintainer choice. The configural and means_metric cases add a caution about the untested rung that a contrast needs.
- AC4: the block runs from `Invariance ladder` to the line before `# Profile`. Over the 14 cases, the widest line is 75 at width 77 and 79 at width 80, with `nchar(type = "width")`. No line is over the width. The M130 test "every line of the ladder block fits the console width at 77 and 80 columns" asserts this.
- AC5: the re-knitted ladder block (lines 408 to 421) has 14 `#>` lines, the widest 75 characters, and none over 80. `tools/check-vignette-staleness.R` exits 0 and reports all 7 vignettes up to date. The `.orig` prose from the paragraph before `by_gender` to the end of the ΔCFI paragraphs, and the gating bullet, were read in full. Each sentence about the print matches the knitted output. The prose says that the `dcfi` values stay in `by_gender$invariance$table$dcfi` and do not print. The `@param invariance`, `@details` and `@return` roxygen was searched for print, verdict, dcfi and ladder, and each hit matches the new output. One `@details` source line is 136 characters (finding P1 below). That is a source wrapping defect, and the sentence is true.
- AC6: `devtools::test()` at `eb1b6f0d` gave FAIL 0, WARN 9, SKIP 1, PASS 9948. `devtools::check(args = "--no-manual")` gave Status OK with 0 errors, 0 warnings and 0 notes. NEWS.md replaces the M127 bullet with two bullets that describe the change. Finding N3 shows one NEWS sentence false in a non-UTF-8 locale: "at 77 and 80 columns every line of the block fits". Under `LC_ALL=C`, Δ prints as `<U+0394>`, and the head block has lines of 81 to 84 characters at both widths. Master's block had lines up to 99 characters in the same locale. Not ticked, pending the gate.
- AC1 correction: finding N6 shows the AC1 clause "name any rung rejected above the required rung" failing when the groups are not comparable. A planted table through `sem_verdict_facts()` (required metric, metric and scalar both rejected) prints no line that names the scalar rejection, with or without a requested contrast. Master's print and the stored verdict also omit it. The AC1 box is unticked, pending the gate.

Consistency gate: `cairn_validate.py` passes, with one advisory (14 tasks over the split tripwire). `document()` with `cli.width = 500` leaves no diff and prints no `resolve link` line. `pkgdown::check_pkgdown()` finds no problems. README.Rmd and README.md are unchanged. The newest verdict push runs of `R-CMD-check.yaml` and `test-coverage.yaml` on master (`af167d0e`) are success. `tools/check-master-red-alert.R`, `tools/master-red-alert-dryrun.R` and `tools/check-branch-protection.R` exit 0. No DESIGN principle changed.

Reviewers: [S] blame-history reported S1 to S3. [S] prior-review reported P1 and P2 (evidence: this file's first pass; no PR review comments exist). [O] diff-bug reported N1 to N14 and found no defect in what the print says about the statistics. Findings, most severe first, with dispositions to be set at the gate:
- N6: the non-comparable arm does not name a rejection above the required rung (AC1 wording, above).
- N3 (with AC6 above): the NEWS width sentence is false in a non-UTF-8 locale. The tests skip there.
- N1 = P1: the `@details` roxygen line at `R/ssm_sem.R:1294` is 136 characters. T10 claimed this fix.
- N2 with S2 and N5: the fallback for an `invariance` element without `required` or `alpha` prints only the stored verdict and drops the profiles, contrast and `ssm_analyze()` cautions. Only hand-modified objects reach it, and its test uses a comparable case only.
- N4 = P2: `print()` still infers the vacuous metric rung from a note on the metric row and does not check it against `inv$comparable`. No current code writes another note there.
- N13: the vignette gating bullet omits "and a rung has a `dcfi` value".
- N14: reworded `.orig` prose lines are not rewrapped (line 357 is 102 characters).
- N7: when a lower rung is untestable and a higher rung is rejected, only the rejection is named. Same as master.
- N9: a retained strict-tier scalar verdict does not say the metric rung holds by construction. Same as master, and the table note says it.
- N10: `Also:` says "this contrast" when no contrast was requested. Master's wording.
- N11: p prints at 4 digits and is compared unrounded, so a p just under .05 could print as 0.0500. Same as master, not run.
- N8: the untestable arm repeats "comparability cannot be established" three times.
