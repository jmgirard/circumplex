...
- [ ] T5: Write the `?axes_reliability` sentence next to the sentence saying that the warning names the worst estimate (`R/axes_reliability.R` ~730). Run `devtools::document()`. Run the AC4 grep and record the disposition of each hit in the work log.
- [ ] T6: Classify every `test_that()` in the file as CRAN-live or CRAN-skipped, with its ground. Draft and append the D-entry. Write the header posture block. Remove `skip_on_cran()` from the anchor-list test (~689). Run the file with `NOT_CRAN=false` and record its skip list.
