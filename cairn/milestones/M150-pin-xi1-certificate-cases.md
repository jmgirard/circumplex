...
- [ ] T5: Measure B's estimate as AC5 states, on macOS arm64 and in the `tools/arm64` image. Record each machine and value in the work log. Rewrite the sentence at `R/axes_reliability.R` ~731-736 to match, and run `devtools::document()`.
- [ ] T6: Sweep the comments for `xi1` in both files: the header (~145-174), the `cert_dd_vs_exact()` block (~626-662), B's test and the oracle comment (~149-156). Record each hit's disposition in the work log.
