# bliton2019 — the IIT-C and its normative descriptive statistics

**Provenance.** Ingested 2026-08-06 by M72 from
`cairn/references/sources/bliton2019.pdf` (gitignored), 18 pages, printed
pp. 1–18 (the article is continuously paginated per article). Pagination: the
article's own page numbers. The PDF is **born-digital** (`Creator: Adobe
InDesign CS5.5`, `Producer: Adobe PDF Library 9.9`), not an OCR scan, so
M42-D1's two-channel requirement does not fire; Table 1 was read in both the
text layer and a layout-preserved rendering, which matters because it packs
three independent measure blocks side by side and only the leftmost is the
IIT-C.
...
**Citation.** Bliton, C. F., & Pincus, A. L. (2019). Construction and validation
of the Interpersonal Influence Tactics Circumplex (IIT-C) Scales.
*Assessment*, 1–18. DOI 10.1177/1073191119864661.

**Role.** The published source for every shipped IIT-C norm value.

**Note-only rows** (published by the source, not shipped): the article's
Table 5 reports a second, Study 2 sample (N = 608) that the package does not
ship.

## Extracted values

The IIT-C items are distributed in an online appendix rather than the article,
so item-to-scale assignment is not verifiable from the shelf copy; angles are
...
| Reference | — | Bliton & Pincus (2019) | p. 1, journal header 'Assessment / 1-18 / (c) The Author(s) 2019' with the running head 'Bliton and Pincus'; Table 1, p. 7 is the venue of the values themselves |
