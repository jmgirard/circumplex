...
- [zimmermann2017.md](zimmermann2017.md) — Zimmermann & Wright (2017), *Beyond description in interpersonal construct validation* — SSM estimator accuracy and sample-size guidance; traces to `vignettes/ci-accuracy.Rmd` and the `jz2017` sample.
- [browne1992a.md](browne1992a.md) — Browne & Cudeck (1992), *Alternative ways of assessing model fit* — the RMSEA cutoffs (0.08 reasonable, greater than 0.1 would-not-employ) AND eqs. 13/14, which `cpm_fit()` implements natively; traces to `R/cpm_fit.R:1039-1060,1085`, `R/ssm_ci_accuracy.R:1014-1023`, `R/ssm_ci_oop.R` and `vignettes/evaluating-circumplex-structure.Rmd`, still page 1 of the split.
- [browne1992.md](browne1992.md) — Browne (1992), *Circumplex models for correlation matrices* — the full specification of the CPM `cpm_fit()` estimates (eqs. 1–8, 30–48), plus a parameter map, five printed errata, and three departures the code takes from the paper; traces to `R/cpm_fit.R`, `R/cpm_oop.R:123,162`, `R/ssm_ci_accuracy.R:169-170`, `tests/testthat/helper-cpm-oracles.R` and three vignettes (`evaluating-circumplex-structure`, `sem-based-ssm-analysis`, `sem-latent-contrasts`).
...
- [acton2004.md](acton2004.md) — Acton & Revelle (2004), *Evaluation of ten psychometric criteria for circumplex structure* — the four criteria `fit_structure()` implements (Fisher eq. 6, Gap eq. 2, VT2 eq. 8, RT eq. 9), the Eq. 6 printed-vs-prose split, and the published nv = 64/128 cutoffs the shipped nv = 8 constants are **not** taken from; traces to `R/fit_structure.R`, `R/fit_structure_oop.R`, `data-raw/structure-test-cutoffs.R` and `vignettes/structure-tests.Rmd`.
- [wendt2019.md](wendt2019.md) — Wendt et al. (2019), *The latent structure of interpersonal problems* — the real-data benchmark for the fixed-angle circumplex CFA (RMSEA .075–.111, g–agency ≈ −.29 across four samples); traces to `vignettes/sem-based-ssm-analysis.Rmd:36,164,384` and `vignettes/sem-latent-contrasts.Rmd:257-259,290-299,340`. Context for the strict tier, never an oracle.
- [hu1999.md](hu1999.md) — Hu & Bentler (1999), *Cutoff criteria for fit indexes in covariance structure analysis* — the SRMR .08 and CFI/TLI .95 cutoffs; traces to `R/ssm_ci_accuracy.R:1023`, `R/ssm_ci_oop.R`, `R/cpm_oop.R:187-188` and `vignettes/evaluating-circumplex-structure.Rmd`, still page 1 of the split.
- [hubert1987.md](hubert1987.md) — Hubert & Arabie (1987), *Evaluating order hypotheses within proximity matrices* — the randomization order test behind `fit_structure()`'s RANDALL: the normalized agreement index (A−D)/(A+D+T) and its exact / (M+1)/(N+1) randomization p-value; traces to `R/fit_structure.R` (`structure_randall()`, `structure_randall_test()`), `tests/testthat/test-fit_structure.R` and `vignettes/structure-tests.Rmd`.
...
- [markey2009.md](markey2009.md) — Markey & Markey (2009), *A brief assessment of the interpersonal circumplex: The IPIP-IPC* — supplies the IPIP-IPC **item assignment and anchors** (Appendix, p. 360) and the shipped N = 274 (Study 2, p. 357), but publishes octant M/SD only for a *different* sample (Study 1 combined, p. 354), so the shipped M/SD have no identified source. Traces to `data/ipipipc.rda`, `data-raw/ipipipc.R`.
...
- [sodano2006.md](sodano2006.md) — Sodano & Tracey (2006), *Interpersonal traits in childhood: Development of the Child and Adolescent Interpersonal Survey* — the published source for both CAIS norm samples (child, Table 2, p. 323; adult, Table 4, p. 325), for the octant target angles (Table 3, p. 324) and for the item-to-octant grouping (Table 1, p. 322), whose unequal-size blocks the shipped eight-cycle item map did not match until M74 rekeyed it (corrected 2026-08-07); records the article's own 204-vs-213 child sample-size split. Traces to `data/cais.rda`, `data-raw/cais.R`.
- [hopwood2008.md](hopwood2008.md) — Hopwood, Pincus, DeMoor & Koonce (2008), *Psychometric characteristics of the Inventory of Interpersonal Problems–Short Circumplex (IIP–SC) with college students* — the published source for iipsc's **sample 1** (Table 1, p. 616, N = 872 pooled across two college samples), whose octant values are four-item sums that `data-raw/iipsc.R` divides by 4; the package credited these norms to a 2011 publication until M74 corrected the `Reference` to 2008 (corrected 2026-08-07). Traces to `data/iipsc.rda`, `data-raw/iipsc.R`.
- [soldz1995.md](soldz1995.md) — Soldz, Budman, Demby & Merry (1995), *A short form of the Inventory of Interpersonal Problems Circumplex Scales* — the IIP-SC's originating paper and the published source for iipsc's **sample 2** (Table 4, p. 59, Generic Outpatient sample, n = 106) and its item-to-octant grouping (Table 2, p. 57); the first shelf source that is an OCR scan, so M42-D1's two-channel rule fires and neither channel is the shelved text layer. Traces to `data/iipsc.rda`, `data-raw/iipsc.R`.
- [horner2024.md](horner2024.md) — Horner, Locke & Hulsey (2024), *Assessing affective dimensions of the interpersonal circumplex: Development and validation of the Interpersonal Emotion Inventory* — the published source for both IEI norm samples (Study 1 N = 1223 and Study 2 N = 278, printed side by side in Table 1, p. 6); also covers the author's IEI norms page, shelved as `locke_iei_norms_2026-08-07.html`, which republishes the sample-1 table and is M74-D1's chosen `URL` anchor. The 64 items are supplemental, not in the article. Traces to `data/iei.rda`, `data-raw/iei.R`.
- [trucco2013.md](trucco2013.md) — Trucco, Wright & Colder (2013), *A revised Interpersonal Circumplex Inventory of Children's Social Goals* — the published source for all three IGI-CR norm samples printed side by side in Table 3 (p. 102: combined N = 387, males n = 174, females n = 213), for the octant target angles (Table 5, p. 104) and for the numbered item-to-octant assignment (Appendix, p. 111). Traces to `data/igicr.rda`, `data-raw/igicr.R`.
- [horowitz2003.md](horowitz2003.md) — Horowitz, Alden, Wiggins & Pincus (2003), *Inventory of Interpersonal Problems manual* (3rd ed., Mind Garden) — the published source for **both** IIP instruments' norms and item keys, so the page carries two instrument-tagged audit blocks: the IIP-64's three norm groups (Table 4.4, pp. 27–29, raw sums divided by 8) and item map (Appendix A, pp. 57–59), and the IIP-32's (Table F.5, p. 91, divided by 4; Appendix H, pp. 101–102). Records that Table F.5 sits at the end of Appendix F and is omitted from the manual's own contents listing, that the manual prints no octant angles, and the Mind Garden credit line the reproduction permission requires. Traces to `data/iip32.rda`, `data/iip64.rda`, `data-raw/iip32.R`, `data-raw/iip64.R`, `R/instrument_data.R`.
- [forward-source-prospects.md](forward-source-prospects.md) — Forward-looking source prospects (M46) — a supply-push survey synthesis note triaging shelved sources (nagy2019, weide2021, rogoza2021, tracey2000; wester2026 added 2026-09-19 as E5, an SPMC-E application folded into the CPM covariate-extension candidate), each mapped to a `candidate` ROADMAP row; traces to nothing yet — per-source pages owed only on graduation (D-024).
- [nagy2019.md](nagy2019.md) — Nagy, Etzel & Lüdtke (2019), *Integrating covariates into circumplex structures* — the figure conventions (Figures 1, 3, 4 and 6; the p. 415 reading of the tangent and distance lines; Appendices B and C by heading) that section 7 of the visualization vignette and the M142 cartesian canvas follow; figure conventions only, no oracle. Traces to `vignettes/advanced-visualization.Rmd.orig` section 7, `R/coord_circumplex.R:49,137` and `R/ssm_plot.R:648`.
- [plain-vignettes.md](plain-vignettes.md) — Plain-English vignettes (M123) — the reader profile, the writing rules, the precision list, the prose-sweep definition and a ledger per milestone for the vignette pass of M123 to M125; traces to `tools/prose-sweep.R`, `tests/testthat/test-prose-sweep.R` and the rewritten vignettes.
