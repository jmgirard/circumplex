...
| SEM | E26: "plane factors are fixed isotropic and orthogonal" | Kept as a base claim | The generated syntax fixes them only under the scaled tier, and the strict tier frees the factor covariance. A form-only pass does not change the claim, and a later content fix scoped it to the scaled tier (corrected M129). |
...
| SEM | VE2 (the article's ΔCFI direction), VE5 (the E26 bullet), VE20 | Rejected | Base claims that a form-only pass keeps. A later content fix corrected VE5 (corrected M129). |
...
| structure | Claim audit A25: "withholds its displacement" and "withholds the displacement interval" | Kept as a base claim | The `accuracy` chunk output prints the displacement and its interval with a not-interpretable note (`R/ssm_oop.R`). A form-only pass does not change the claim. A later content fix corrected it (corrected M129). |
...
| structure | RD4, RD5: "a 'weak' classification" and "`cpm_fit()` commits to the theoretical angles" | Kept as base claims | Print shows "not clearly supported" and summary shows "unsupported" (`R/fit_structure_oop.R`). The default quasi-circumplex model estimates the angles. A later content fix corrected both (corrected M129). |
