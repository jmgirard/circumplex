...
- Radial axis labels obscured by data layers (2026-07-18, found by M38's T6 render-and-inspect sweep): the amplitude labels are panel furniture drawn beneath the geom layers, so a marker or arrowhead can cover them — in `advanced-visualization.Rmd` the `occasions-path-wrapper` figure's `0.50` label is unreadable under the T3 marker and arrowhead, `occasions-path` hides `0.6` the same way, and `individuals` prints its labels over a dense scatter. M32 solved label-vs-*spoke-label* collision by moving the radial axis into the widest spoke gap; this is label-vs-*data* and needs a different remedy (a halo or background behind the labels, or drawing the radial guide above the layers). Not absorbed into M38, which owns the rim ring only.
