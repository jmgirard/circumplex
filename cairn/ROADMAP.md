...
| M11 | Boundary-coverage hardening + test-suite tidiness | in-progress | — | normal | milestones/M11-boundary-coverage-hardening.md |
...
- Continuous / infrastructure refactors (fold into the milestone that next touches the code): dedup Group/Measure/Label construction; move degree/radian/contrast classes onto vctrs/S7; add R-devel to CI matrix; statistical-core coverage tracking; 0-vs-360 pole-snap alignment (cosmetic); analytic-CI Hessian recomputation (minor perf) (legacy ROADMAP "Continuous / infrastructure track" + deferred `/code-review` findings). _(2026-07-12: the boundary-condition test suite and the `test-RcppExport.R.R` rename from this list were absorbed into M11.)_
