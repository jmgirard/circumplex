...
- Bare-`coord_circumplex()` vignette figure (2026-07-18): the `coord-bare` chunk in `vignettes/advanced-visualization.Rmd` renders spokes at ggplot2's default breaks (0/100/200/300), asymmetric and colliding with the amplitude labels. The prose at :104–108 documents this as intentional (it motivates what `ggcircumplex()` adds), but it is the reader's first build-from-parts figure and it reads as broken. Revisit whether the example should carry `scale_x_continuous(breaks = octants())` and make the point in prose instead. Teaching-figure quality, not a code defect. (The missing outer ring at explicit `amax` was routed to a hotfix the same day.)
