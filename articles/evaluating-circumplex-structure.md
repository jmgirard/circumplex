...
data. The section *When a fit sits at a boundary*, below, explains the
first. The second means that the intervals rest only on the resamples
that were kept.
...
that *When a fit sits at a boundary* explains.
...
same ill-conditioned Hessian warning that *When a fit sits at a
boundary* explains.
