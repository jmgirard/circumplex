...
`Zeta` is each scale’s communality index, which the third figure uses.

### Estimated angles as ticks on the rim

The first figure draws each estimated angle as a short radial tick just
inside the rim. The canvas’s spokes sit at the theoretical angles. A
tick is a path with two points at the same displacement, one at
amplitude 0.94 and one at amplitude 1.
[`geom_ssm_path()`](http://circumplex.jmgirard.com/reference/geom_ssm_path.md)
draws it with `group = Scale`. The canvas is
[`ggcircumplex()`](http://circumplex.jmgirard.com/reference/ggcircumplex.md)
with the fit’s `Angle_theory` column as its angles, so the spokes and
labels sit where the theory put each scale.

``` r

ticks <- data.frame(
  Scale = rep(cpm$results$Scale, each = 2),
  amplitude = rep(c(0.94, 1), times = nrow(cpm$results)),
  displacement = rep(cpm$results$Angle, each = 2)
)
ticks$Scale <- factor(ticks$Scale, levels = cpm$results$Scale)

ggcircumplex(angles = cpm$results$Angle_theory, labels = PANO(), amax = 1) +
  geom_ssm_path(
    data = ticks,
    mapping = aes(
      amplitude = amplitude, displacement = displacement,
      group = Scale, color = Scale
    ),
    linewidth = 1.5
  )
```
...
Read the figure this way: the ticks are the angles
[`cpm_fit()`](http://circumplex.jmgirard.com/reference/cpm_fit.md)
estimated, and the labelled spokes are the theoretical angles. A tick
that sits on its spoke is a scale the model placed where the theory did.
PA’s tick sits on its spoke by construction, because its angle was
fixed. In this sample the other ticks drift clockwise from BC round to
NO, and the NO tick lands near the LM spoke. The amplitude rings carry
no meaning in this figure. Radius here only keeps the ticks at the rim.

### Measure vectors on the same rim

Gurtman (1992) draws each external measure as a vector from the origin.
Its direction is the measure’s displacement and its length is the
measure’s amplitude.
[`ssm_analyze()`](http://circumplex.jmgirard.com/reference/ssm_analyze.md)
supplies both. The second figure adds five personality-disorder measures
from `jz2017` as such vectors, on the same rim ticks. Each vector is a
two-row path from amplitude 0 to the measure’s `a_est` at its `d_est`.
The ticks are scaled by the canvas’s `amax` so that they stay just
inside the rim.
...
vectors <- data.frame(
  Measure = rep(ssm$results$Label, each = 2),
  amplitude = c(rbind(0, ssm$results$a_est)),
  displacement = rep(ssm$results$d_est, each = 2)
)

ggcircumplex(angles = cpm$results$Angle_theory, labels = PANO(), amax = 0.3) +
  geom_ssm_path(
    data = ticks,
    mapping = aes(
      amplitude = 0.3 * amplitude, displacement = displacement,
      group = Scale
    ),
    linewidth = 1.5
  ) +
...
This figure combines three sets of angles, and they do not share a
model. The spokes are the theoretical angles. The ticks are the angles
the circular process model estimated. The vectors are SSM displacements,
and the SSM was computed with the theoretical angles, not the estimated
ones. So the spoke and the tick for the same scale come from different
models, and the vectors come from a third: the SSM computed on the
theoretical angles. A vector that points at a tick does not mean that
the measure loads on that scale’s latent position. Likewise, a vector’s
length is the measure’s amplitude, not its correlation with the latent
circumplex. The amplitude is the cosine curve’s amplitude in the
measure’s correlations with the eight scales, in the correlation metric
that
[`ssm_analyze()`](http://circumplex.jmgirard.com/reference/ssm_analyze.md)
reports. It is not a loading on the latent circle. To place a measure on
the estimated circle itself, you would fit it inside the model, which
[`cpm_fit()`](http://circumplex.jmgirard.com/reference/cpm_fit.md) does
not do.

### The fitted correlation function

The third figure is linear. The fit’s `corfun` element is the estimated
