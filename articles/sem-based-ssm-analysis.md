...
#> ΔCFI: Cheung & Rensvold (2002) criterion, alpha = .01, two-group ML simulation
#>   scope. The cutoff is NOT validated here (robust CFI), so the value is
#>   descriptive only, with no binary verdict. It is secondary and reported only:
#>   the verdict below gates on the nested chi-square difference test alone.
#> Verdict: metric invariance rejected (Δχ²(14) = 54.78, p < 0.0001, alpha =
#>   0.05): these groups cannot be compared on this instrument's latent metric.
#> The requested latent contrast was therefore not computed. The rows below are
#> each group's separate (configural) latent profile. The observed-score contrast
#> from ssm_analyze() answers a different question and remains available.
#> 
...
The ladder table also carries `dcfi`, the change in the comparative fit
index (CFI) from the previous fitted rung. CFI compares the model’s fit
with that of a baseline model in which the variables are uncorrelated,
and values near 1 mean good fit. `dcfi` is Cheung and Rensvold’s (2002)
secondary criterion. Its general rule rejects an invariance step when
CFI falls by more than .01. It is **reported, never gating**.
Comparability, the verdict, and the model that the estimates are taken
from are decided by the nested test alone. The two criteria can
disagree, and neither is a tiebreaker for the other. A change in CFI is
insensitive to sample size, but the nested test is not. So in a large
sample the nested test can reject a step whose CFI barely moves.
...
envelope. The default estimator is `"MLR"`. So the ladder above prints
its `dcfi` value marked as outside that scope, with no verdict attached
and the note naming which condition applies. The same withholding covers
a non-ML estimator such as `"GLS"`. Its CFI is plain-named, but its fit
function is not the one that the criterion was simulated under. That is
a deliberate refusal rather than a gap. Extending the cutoff to a robust
...
- **Invariance gating is a modeling decision** with a default test, not
  ...
  question. The secondary `dcfi` criterion printed beside the nested
  test gates nothing. Its .01 cutoff is validated only for two-group,
  ML, multivariate-normal fits, for Type I error only. The package
